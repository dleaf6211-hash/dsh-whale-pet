...
function Update-UI {
  ...
  if ($s.notice -ne $null -and $s.notice.id -ne $null) {
    ...
    if ($s.notice.at -gt 0 -and ($nowMs2 - [long]$s.notice.at) -lt 6000) { $noticeFresh = $true }
  }
  if ($noticeFresh -and $s.dnd -eq $true) {
    $st = ''
    if ($s.notice -ne $null -and $s.notice.status -ne $null) { $st = [string]$s.notice.status }
    if ($st -notin @('approval', 'needs_help')) { $noticeFresh = $false }
  }
  if ($noticeFresh -and [long]$s.notice.id -eq $script:noticeDismissed) { $noticeFresh = $false }
  $spk = $false
  ...
  if ($spk -and $script:speechSuppressTick -ge 0 -and $script:speechSuppressTick -eq [long]$s.speech.audioTick) { $spk = $false }
  if ($noticeFresh) {
    $nt = 'Notice'
    ...
    if ($s.notice.cost -ne $null -and [string]$s.notice.cost -ne '') {
      [void]$rows.Children.Add((New-Row ([string]$s.labels.rowCost) ([string]$s.notice.cost)))
    }
    if ([string]$s.notice.in -ne '' -and [string]$s.notice.in -ne '0') { [void]$rows.Children.Add((New-Row ([string]$s.labels.rowIn) ([string]$s.notice.in))) }
    if ([string]$s.notice.out -ne '' -and [string]$s.notice.out -ne '0') { [void]$rows.Children.Add((New-Row ([string]$s.labels.rowOut) ([string]$s.notice.out))) }
    if ([string]$s.notice.cache -ne '' -and [string]$s.notice.cache -ne '0') { [void]$rows.Children.Add((New-Row ([string]$s.labels.rowCache) ([string]$s.notice.cache))) }
    [void]$btns.Children.Add((New-Btn '知道了' { $script:noticeDismissed = $script:lastNoticeId; $script:view = 'balance'; Update-UI }))
    if ([long]$s.notice.id -ne $script:lastNoticeId) {
      $script:lastNoticeId = [long]$s.notice.id
      if ($s.voiceEngine -eq 'pet' -and $s.voiceEnabled -ne $false -and $s.dnd -ne $true -and $noticeDir -ne '') {
        $cands = @()
        for ($i = 0; $i -lt 3; $i++) {
          $vp = Join-Path $noticeDir ('notice-' + [string]$s.notice.status + '-' + $i + '.wav')
          if (Test-Path $vp) { $cands += $vp }
        }
        $vp0 = Join-Path $noticeDir ('notice-' + [string]$s.notice.status + '.wav')
        if (Test-Path $vp0) { $cands += $vp0 }
        if ($cands.Count -gt 0) {
          if ($script:voicePlayer -ne $null) { try { $script:voicePlayer.Stop() } catch { } }
          $sp2 = New-Object System.Media.SoundPlayer($cands[(Get-Random -Maximum $cands.Count)])
          $sp2.Play()
    ...
    [void]$btns.Children.Add((New-Btn '知道了' { $script:speechSuppressTick = $script:lastTick; $script:view = 'balance'; Update-UI }))
    if ($s.speech.audioTick -gt 0 -and [long]$s.speech.audioTick -ne $script:lastTick) {
      if (Test-Path $speechWav) {
        $script:lastTick = [long]$s.speech.audioTick
    ...
    $title.Text = [string]$s.labels.titleUsage
    $big.Text = ''
    if ($s.tokens -ne $null) {
      [void]$rows.Children.Add((New-Row ([string]$s.labels.rowCalls) ([string]$s.tokens.calls)))
      [void]$rows.Children.Add((New-Row ([string]$s.labels.rowIn) ([string]$s.tokens.input)))
      [void]$rows.Children.Add((New-Row ([string]$s.labels.rowOut) ([string]$s.tokens.output)))
      [void]$rows.Children.Add((New-Row ([string]$s.labels.rowCache) ([string]$s.tokens.cacheRead)))
    }
  } else {
    ...
    if ($s.costRows -ne $null) {
      $costArr = @(
        @{ l = [string]$s.labels.rowSession; v = [string]$s.costRows.session },
        @{ l = [string]$s.labels.rowMonthly; v = [string]$s.costRows.monthly },
        @{ l = [string]$s.labels.rowDiff; v = [string]$s.costRows.diff }
      )
  ...
  if (-not $noticeFresh -and -not $spk) {
    $toggleText = '看用量'
    if ($script:view -eq 'usage') { $toggleText = '看余额' }
    [void]$btns.Children.Add((New-Btn $toggleText { if ($script:view -eq 'balance') { Set-View 'usage' } else { Set-View 'balance' }; Update-UI }))
...
export function apply(ctx, config) {
  ...
  const state = {
    ...
    taskQueue: [],
    taskDoneId: 0,
    usage: { calls: 0, inputTokens: 0, outputTokens: 0, cacheReadTokens: 0, lastModel: '' },
    monthlyUsage: { month: '', inputTokens: 0, outputTokens: 0, cacheReadTokens: 0, costCny: 0 },
  ...
  function costOf(input, output, cacheRead) {
    const price = currentPrice()
    const cr = typeof cacheRead === 'number' && isFinite(cacheRead) ? cacheRead : 0
    return Math.round(((input / 1000000) * price.input + (output / 1000000) * price.output + (cr / 1000000) * price.cacheHit) * 100000) / 100000
  }
  ...
  function extractUsage(chunk) {
    ...
    return { input: inp, output: outp, total: nt, cacheRead: ncr }
  }
  ...
  async function fetchBalance() {
    ...
    try {
      ...
      // 低余额检测:< 5 元触发一次提醒(version 递增驱动客户端弹窗);设置里可关
      const total = rows.length > 0 ? parseFloat(rows[0].total) : NaN
      if (state.settings.lowBalanceAlert !== false && isFinite(total) && total < 5) {
  ...
  const routes = [
    ...
    route('/api/dsh-whale-pet/topup-qr', async (req, res) => {
      try {
        if (qrCache !== null && Date.now() - qrCacheAt < 3600000) {
          writeJson(res, 200, { ok: true, base64: qrCache, mime: 'image/png' })
          return
        }
        ...
        if (body === null || body.length < 100 || body[0] !== 0x89 || body[1] !== 0x50 || body[2] !== 0x4e || body[3] !== 0x47) {
          writeJson(res, 200, { ok: false, error: 'qr-fetch-failed' })
          return
        }
  ...
  function pushNotice(item) {
    // 去重:① 同一 turn 同状态同类别只发一次(needs_help 例外:同轮不同问题各弹一次)
    // ② 连续 completed 90 秒内只留第一条(仅 agent 类、仅同会话)
    const lastItem = state.taskQueue.length > 0 ? state.taskQueue[state.taskQueue.length - 1] : null
    const sameTurnRule = lastItem !== null && lastItem.status === item.status && lastItem.kind === item.kind &&
  ...
  ctx.on('agent/status', (payload) => {
    try {
      ...
      // 会话成本账本跨重启连续:mainSessionId 仅在为空时取当前会话,
      // 恢复出的历史账本继续累加,不随每次重启的新会话 id 清零
      if (state.mainSessionId === '') {
        if (typeof sid === 'string' && sid !== '') state.mainSessionId = sid
      }
  ...
  ctx.on('approval/request', (req, next) => {
    try {
      // 同一 turn 已弹过「需要协助」时不再重复弹审批(feedback-plan 第 1 项修过的 bug)
      const lastItem = state.taskQueue.length > 0 ? state.taskQueue[state.taskQueue.length - 1] : null
      const sameTurnNeedsHelp = lastItem !== null && lastItem.status === 'needs_help' &&
        typeof lastItem.announceTurn === 'number' && lastItem.announceTurn === state.turnSeq
  ...
  ctx.on('tools/pre-execute', (exec, next) => {
    try {
      ...
      if (tname.indexOf('question') >= 0 || tname.indexOf('ask_user') >= 0 || tname.indexOf('askuser') >= 0) {
        ...
        const title = typeof q === 'string' && q !== '' ? '提问：' + q : '需要协助'
  ...
  ctx.on('subagent/end', (payload) => { announceChildDone('subagent', payload) })
    ...
    if (jobsSvc && typeof jobsSvc.onJobDone === 'function') {
      const jd = jobsSvc.onJobDone((snapshot) => {
        try {
          ...
          const title = label !== '' ? label.slice(0, 24) : (st === 'completed' ? '后台任务完成' : st === 'failed' ? '后台任务失败' : '后台任务中断')
          pushNotice({
    ...
    function checkPetHeartbeat() {
      try {
        ...
        if (existsSync(hbPath)) {
          ...
          alive = isFinite(t) && Date.now() - t < 10000
        }
        // 启动宽限:拉起后 15 秒内不因心跳缺失判定死亡(宠物启动+首次心跳需要时间)
        if (!alive && Date.now() - lastPetLaunchAt < 15000) alive = true
        // 关闭宽限:显式 kill 后 15 秒内不因旧心跳残留判定存活(防止关掉又"复活")
        if (alive && Date.now() - lastPetKillAt < 15000) alive = false
        ...
        for await (const chunk of upstream) {
          try {
            ...
            if (u !== null) {
              ...
              state.monthlyUsage.costCny = Math.round((state.monthlyUsage.costCny + costOf(u.input, u.output, u.cacheRead)) * 100000) / 100000
              if (state.mainSessionId !== '') {
                let se = state.sessionCosts.get(state.mainSessionId)
                if (se === undefined) {
                  se = { inputTokens: 0, outputTokens: 0, cacheReadTokens: 0, costCny: 0 }
                  state.sessionCosts.set(state.mainSessionId, se)
                }
                se.inputTokens += u.input
                se.outputTokens += u.output
                se.cacheReadTokens += u.cacheRead
                se.costCny = Math.round((se.costCny + costOf(u.input, u.output, u.cacheRead)) * 100000) / 100000
              }
