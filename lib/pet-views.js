      ...
      if ($s.voiceEngine -eq 'pet' -and $s.voiceEnabled -ne $false -and $s.dnd -ne $true) {
        ...
        if ($isNoticeLike) {
          ...
          if ($sw -ne '' -and (Test-Path $sw)) {
            ...
            if ($curSpeech -ne '' -and $curSpeech -eq $globSpeech) {
              $fp = ''
              try {
                $fi = Get-Item $speechWav
                $fp = ([string]$fi.LastWriteTimeUtc.Ticks) + ':' + ([string]$fi.Length)
              } catch { $fp = '' }
              if ($fp -ne '' -and $fp -ne $script:lastSpokenFp) { $script:lastSpokenFp = $fp; $play = $speechWav }
            }
          }
        }
        if ($play -ne $null) {
